...
`betaregscale()` — the internal `check_response()` step is skipped:
...
The default midpoint interval type means that each observation is centered
in its interval. For example, a score of 67 on a 0--100 scale yields
