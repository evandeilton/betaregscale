...
$`Y_i \sim \text{Beta}(\mu_i, \sigma_i)`$ has expected value and
variance given by:
``` math
\text{E}(Y) = \mu, \quad \text{Var}(Y) = \mu(1-\mu)\sigma
```

Both the mean and dispersion can be modeled via link functions ($`g`$
and $`h`$) allowing for covariate-dependent heteroscedasticity:
``` math
...
Raw scores $`y_i^{*} \in \{0, \dots, K\}`$ are mapped to the unit
interval as $`y_i = y_i^{*}/K`$, with uncertainty intervals
$`[l_i, u_i] = [y_i - 1/(2K), y_i + 1/(2K)]`$.

Let $`\delta_i \in \{0, 1, 2, 3\}`$ indicate the censoring type (exact,
